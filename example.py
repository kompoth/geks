...
hm = geks.Hexmap(default_value, dims=(12, 8))
...
for he, value in hm.map.items():
    mpl.plot_hex(
        he, fill=value['blocked'] or value['price'] > 1, 
...
path, nstep, nit = geks.dijkstra_path (
    hm, start, target, distance=40, 
    block_func=lambda x: hm.map[x]['blocked'],  # walls condition
    cost_func=lambda x: hm.map[x]['price']     # cost of movement
)
mpl.plot_path(path, edgecolor='darkblue')
